 ...
 */
public class SaltWaterAsync extends AsyncTask<String, Void, Integer> {

    SpeciesActivity.SaltWaterFragment activity;
    ArrayList<SpeciesModel> SpeciesList = new ArrayList<>();  //safe

    public SaltWaterAsync(SpeciesActivity.SaltWaterFragment fragment, ArrayList speciesList) {
        activity = fragment;
        ...
        try {
          ...
          //  conn.setDoOutput(false);
            ...
            else
            {
              //  parseResult("");
                result = 0;
                EventBus.getDefault().post(BasicEvent.SALT_WATER_ERROR);
            }
